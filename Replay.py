...
class Replay:
    ...
    def goodMorning(self):
        with open(self.data+'morningReplay.txt','r',1,encoding='utf-8') as morningMessage:
            self.morning = [line.strip() for line in morningMessage.readlines()]
    ...
    def goodMorningInAfterNoon(self):
        with open(self.data+'morningReplayInAfterNoon.txt','r',1,encoding='utf-8') as goodAfterNoonMessage:
            self.mAfterNoon = [line.strip() for line in goodAfterNoonMessage.readlines()]
    ...
    def goodNight(self):
        with open(self.data+'sleepGreeting.txt','r',1,encoding='utf-8') as sleepGreetingMessage:
            self.gNight = [line.strip() for line in sleepGreetingMessage.readlines()]
    ...
    def shyTB(self):
        with open(self.data+'shyReplay.txt','r',1,encoding='utf-8') as shyTBMessage:
            self.shyReplay = [line.strip() for line in shyTBMessage.readlines()]
    ...
    def weatherReport(self,keyMessage,day,informationFlag,time):
        ...
        if city in self.cityCodes.keys():
            ...
            responce = urllib.request.urlopen('http://weather.livedoor.com/forecast/webservice/json/v1?city=%s'%code).read()
            #responce = urllib3.PoolManager().request('GET','http://weather.livedoor.com/forecast/webservice/json/v1?city=%s'%cityCode) HTTPsも使える版
